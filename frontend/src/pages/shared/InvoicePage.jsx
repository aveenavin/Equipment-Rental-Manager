    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 relative overflow-hidden">
      ...
      <div className="print:hidden max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-6 relative z-10">
        <div className="flex items-center justify-between">
          <Link to={backPath} className="inline-flex items-center gap-2 text-sm font-medium text-slate-400 hover:text-slate-200 transition-colors bg-slate-900/50 px-4 py-2 rounded-xl border border-slate-800 backdrop-blur-sm">
            <ArrowLeft className="h-4 w-4" /> Back to Rental
          </Link>
          <button
            onClick={() => window.print()}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-primary-600 to-primary-500 hover:from-primary-500 hover:to-primary-400 text-white text-sm font-bold tracking-wide transition-all shadow-lg shadow-primary-500/25 border border-primary-400/20"
          >
      ...
      <motion.div 
        ...
        className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 pb-16 relative z-10 print:px-0 print:pb-0 print:max-w-none"
      >
        <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-3xl overflow-hidden shadow-2xl print:rounded-none print:border-none print:bg-white print:text-slate-900 print:shadow-none print:backdrop-blur-none relative flex flex-col min-h-[500px]">
          
          {/* Decorative Top Accent */}
          <div className="h-2 w-full bg-gradient-to-r from-primary-500 via-emerald-500 to-blue-500 print:hidden" />

          {/* Invoice header */}
          <div className="px-6 sm:px-8 pt-6 pb-5 border-b border-slate-800/60 print:border-slate-200">
            <div className="flex flex-col sm:flex-row items-start justify-between gap-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-br from-primary-400 via-emerald-400 to-blue-500 print:text-slate-900 tracking-widest drop-shadow-sm uppercase">INVOICE</h1>
                  <p className="text-primary-400 font-mono text-sm mt-1 font-semibold print:text-blue-600">{invoice.invoiceNumber}</p>
                  <p className="text-slate-400 text-xs mt-0.5 font-medium print:text-slate-600">Issued: {fmt(invoice.issuedAt)}</p>
                </div>
              </div>
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12">
            ...
            <div className="lg:col-span-8 border-b lg:border-b-0 lg:border-r border-slate-800/60 print:border-slate-200">
              ...
              {/* Bill To / Equipment */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-6 sm:px-8 py-5 border-b border-slate-800/60 print:border-slate-200">
                {/* Bill To */}
                <div className="p-4 rounded-2xl bg-slate-800/30 border border-slate-700/50 print:p-0 print:bg-transparent print:border-none">
                  ...
                  <p className="font-bold text-base text-slate-100 print:text-slate-900">{invoice.customer?.name}</p>
                ...
                <div className="p-4 rounded-2xl bg-slate-800/30 border border-slate-700/50 print:p-0 print:bg-transparent print:border-none">
                  ...
                  <p className="font-bold text-base text-slate-100 print:text-slate-900">{invoice.equipment?.name}</p>
                  <p className="text-sm font-medium text-primary-400 capitalize print:text-slate-600">{invoice.equipment?.category?.replace(/-/g, ' ')}</p>
                  <div className="mt-2 pt-2 border-t border-slate-700/50 print:border-slate-200 space-y-1">
                    <p className="text-xs font-mono font-medium text-slate-400 print:text-slate-500">Duration: {fmt(invoice.rental?.startDate)} → {fmt(invoice.rental?.endDate)}</p>
                    <p className="text-xs font-medium text-slate-400 print:text-slate-500">{invoice.rental?.totalDays} day{invoice.rental?.totalDays !== 1 ? 's' : ''} @ ₹{invoice.rental?.dailyRate}/day</p>
                  </div>
                </div>
              </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b-2 border-slate-800 print:border-slate-300">
                        <th className="pb-2 text-xs font-bold text-slate-400 uppercase tracking-wider print:text-slate-500 pl-4">Description</th>
                        <th className="pb-2 text-xs font-bold text-slate-400 uppercase tracking-wider print:text-slate-500">Details</th>
                        <th className="pb-2 text-xs font-bold text-slate-400 uppercase tracking-wider print:text-slate-500 text-right pr-4">Amount</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800/60 print:divide-slate-200">
                      {invoice.lineItems?.map((item, i) => (
                        <tr key={i} className="hover:bg-slate-800/20 transition-colors print:hover:bg-transparent">
                          <td className="py-2.5 pl-4 text-sm font-medium text-slate-200 print:text-slate-800">{item.description}</td>
                          <td className="py-2.5 text-xs text-slate-500 print:text-slate-500">{item.detail}</td>
                          <td className={`py-2.5 pr-4 text-sm font-bold text-right ${item.amount < 0 ? 'text-emerald-400 print:text-emerald-600' : 'text-slate-200 print:text-slate-800'}`}>
            ...
            <div className="lg:col-span-4 flex flex-col bg-slate-800/10 print:bg-transparent">
              ...
              {/* Totals Summary */}
              <div className="px-6 sm:px-8 py-5 border-b border-slate-800/60 print:border-slate-200">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm font-bold">
                    ...
                    <span className="text-white print:text-slate-900">₹{invoice.totals?.totalAmount?.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm font-bold">
                  ...
                  <div className="mt-4 pt-3 border-t-2 border-primary-500/50 print:border-slate-400 flex justify-between items-center">
                    <span className={`text-base font-black tracking-widest uppercase ${invoice.paymentSummary?.balance > 0 ? 'text-red-400 print:text-red-600' : 'text-emerald-400 print:text-emerald-600'}`}>
                      {invoice.paymentSummary?.balance > 0 ? 'Balance Due' : 'Fully Paid'}
                    </span>
                    <span className={`text-xl font-black ${invoice.paymentSummary?.balance > 0 ? 'text-red-400 print:text-red-600' : 'text-emerald-400 print:text-emerald-600'}`}>
                  ...
                  <div className="space-y-2">
                    {invoice.payments.map((p) => (
                      <div key={p._id} className="flex items-center justify-between p-2.5 rounded-lg bg-slate-800/30 border border-slate-700/30 print:p-0 print:bg-transparent print:border-none print:mb-1">
                        <div>
                          <p className={`text-[11px] font-bold uppercase tracking-wider ${TYPE_COLORS[p.paymentType] || 'text-slate-400'} print:text-slate-600`}>
                            {p.paymentType.replace('_', ' ')} <span className="text-slate-500 lowercase opacity-70">via</span> {p.paymentMethod}
                          </p>
                          <p className="text-[10px] font-medium text-slate-500 mt-0.5 print:text-slate-400">{fmt(p.paidAt)}</p>
                        </div>
                        <span className={`text-xs font-black tracking-wide ${p.direction === 'outbound' ? 'text-red-400 print:text-red-600' : 'text-emerald-400 print:text-emerald-600'}`}>
